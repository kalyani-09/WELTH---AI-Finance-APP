...
import { db } from "@/lib/prisma"; // adjust path
...
export const checkBudgetAlert = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    const results = []; // collect results for output
      ...
      await step.run(`check-budget-${budget.id}`, async () => {
        ...
        if (percentageUsed >= 80 &&
          ...
          try {
            ...
            results.push({ budgetId: budget.id, status: "email_sent", emailResult });
          } catch (err) {
            results.push({ budgetId: budget.id, status: "email_failed", error: err.message });
          }
        } else {
          results.push({ budgetId: budget.id, status: "no_alert", percentageUsed });
        }
      });
    }

    // ✅ return summary so you see real output instead of null
    return results;
  }
);
      ...
      async () => {
        return await db.transaction.findMany({
          where: {
            ...
            status: "COMPLETED",
            OR: [
              { lastProcessed: null },//Never Proceesed Before
  ...
  async ({ event, step }) => {
    ...
    await step.run("process-transaction", async () => {
      ...
      await db.transaction(async (tx) => {
        ...
        //Update last proceesed date and next recurring date
...
function isTransactionDue(transaction) {
  //If no lastprocessed date, trnsaction is due
...
const getMonthlyStats = async (userId, month) => {
  ...
  return transactions.reduce(
    (stats, t) => {
      ...
      if (t.type === "EXPENSE") {
        stats.totalExpenses += amount; // ✅ FIXED
